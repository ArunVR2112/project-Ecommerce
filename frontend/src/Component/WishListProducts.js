...
export default function WishListProduct({product}) {
    ...
    let {removeProductFromWishList,handleQuantityChangeWishList} = useContext(AppContext)
  ...
  return (
    <div className='flex space-x-5 border border-gray-200 mt-3 p-3'>
        ...
        <div className='w-full'>
                <h2>{product.title}</h2>
                <input type="number" value={product.quantity} onChange={(e)=>{handleQuantityChangeWishList(product.id, e.target.value)}} />
                <span>
            <svg
